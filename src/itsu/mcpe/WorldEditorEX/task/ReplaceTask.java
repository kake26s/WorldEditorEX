...
public class ReplaceTask extends Task {
    ...
    public ReplaceTask(Block to, Block from, Level level, Player player, Location loc1, Location loc2) {
        this.to = to;
        this.from = from;
        this.level = level;
        this.player = player;
        ...
        for(int x = x2;x <= x1;++x) {
            for(int y = y2;y <= y1;++y) {
                for(int z = z2;z <= z1;++z) {
                    ...
                    vec.y = y;
                    vec.z = z;
                    if(level.getBlock(vec).getId() == from.getId() && level.getBlock(vec).getDamage() == from.getDamage()) {
                        level.setBlock(vec, to);
                    }
                }
            }
        }
